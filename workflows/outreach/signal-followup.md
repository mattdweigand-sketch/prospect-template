...
4. Build sent, sent_lookup_reference, account, contacts, contacts_complete, tasks, tasks_complete and signal in the packet. tasks must be an explicit list of task objects; use [] only after a complete read found none. Run `python3 scripts/followup_gate.py --packet output/RUN_ID/followup.json` (or `--mode arr_growth`). The gate blocks duplicate subject/message ID and any open task with the configured follow-up prefix. Closed-status exceptions apply only to that prefix test, not an exact duplicate.
