...
4. Build sent, sent_lookup_reference, account, contacts, contacts_complete, tasks, tasks_complete and signal in the packet. IDs are nonblank strings and addresses use supported single-address syntax. tasks must be an explicit list; every task supplies ID, subject, status and description (an empty description is valid). Use [] only after a complete read found none. Missing fields are unusable input; an unfamiliar nonblank status remains open unless explicitly closed by policy. Run `python3 scripts/followup_gate.py --packet output/RUN_ID/followup.json` (or `--mode arr_growth`). The gate blocks duplicate subject/message ID and any open task with the configured follow-up prefix. Closed-status exceptions apply only to that prefix test, not an exact duplicate.
5. Due date uses the sent date in identity.timezone plus followup_signal.due_calendar_days for standard mode, or growth_due_business_days weekdays for ARR mode. Weekdays exclude weekends, not local holidays. Naive or future send timestamps, including later today, block. A due date before today blocks; a due date today is allowed. Do not shift an expired date to work around the gate.
...
In inputs.json, name `handoff`, `packet`, `result` and `mode` (`standard` or `arr_growth`), following [workflows/run.md](../run.md). Bind the task effect's `input` to the packet path. Preflight verifies the reviewed send attribution and current task prerequisites; it does not re-qualify the original drafting signal against today's freshness window.
