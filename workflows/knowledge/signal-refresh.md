...
5. Read changed pages in the configured source watch paths, including relevant product/service documentation and customer evidence. Paths are normalized relative to the configured source root; nested watched directories match complete path components. The report includes added, modified and deleted files, including explicitly watched paths. Deleted entries identify prior_revision/prior_path; a rename is a deletion plus addition. Inspect old content at that prior revision. An unbound deleted page remains visible without permanently preventing an otherwise valid source update; missing claim evidence or required watch files still block. Skip redundant claims and pages without an evidence-backed connection to the configured buyer's work. Each candidate needs one verbatim sentence supporting something useful after a signal. Cap additions at refresh.max_new_rows; record skipped pages and reasons.
...
10. Put complete diffs/postimages, pinned source report and exact effects in 01_review.md. Include SHA-256 expected_after entries for every active factory file the effect will change as specified by workflows/run.md. Linked row/binding changes belong in one coherent configuration effect. Show added/fixed/cut rows, holds, proposed stamps and source revision. Do not include unapproved extra edits in a combined file postimage.
...
In inputs.json, retain `diagnostic: {shared, result, revision}` naming the original factory snapshot, its source report and exact target commit. Set source_revision to the proposed pinned commit and postimages to the final proposed factory directory when configuration changes. Bind configuration effects to that directory. Run `runs.py preflight RUN_ID --source SOURCE_CLONE`; it replays diagnostics against their retained original baseline and separately validates postimages. Advancing source_revision and stamps is an expected proposal change, not diagnostic drift. A no-change report retains diagnostics without postimages or effects.
